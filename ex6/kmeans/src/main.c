...
int main(int argc, char **argv)
{
     ...
     */
    int ret = MPI_Bcast(clusters, numClusters * numCoords, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    if (ret != MPI_SUCCESS)
    {
    ...
    */
    ...
    if (rank == 0)
    {
         ...
         */
        displs[0] = 0;
        recvcounts[0] = (numObjs / size);
        for (i = 1; i < size; i++)
        {
            displs[i] = displs[i - 1] + recvcounts[i - 1];
            if (i == size - 1)
                recvcounts[i] = (numObjs / size) + (numObjs % size);
            else
     ...
     */
    ret = MPI_Bcast(recvcounts, size, MPI_INT, 0, MPI_COMM_WORLD);
    if (ret != MPI_SUCCESS)
    {
        printf("Error: Recvcounts broadcast failed!\n");
        MPI_Finalize();
        return 1;
    }
     ...
     */
    ret = MPI_Gatherv(membership, rank_numObjs, MPI_INT, tot_membership, recvcounts, displs, MPI_INT, 0, MPI_COMM_WORLD);
    if (ret != MPI_SUCCESS)
    {
        printf("Error: Gather failed!\n");
        MPI_Finalize();
        return 1;
    }
