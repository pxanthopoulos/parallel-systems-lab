            ...
            double *clusters)    /* out: [numClusters][numCoords] */
{
    ...
    do
    {
         ...
         */
        int ret = MPI_Allreduce(rank_newClusters, newClusters, numClusters * numCoords, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        if (ret != MPI_SUCCESS)
        {
            printf("Error: Cluster allreduce failed!\n");
            MPI_Finalize();
            exit(1);
        }

        ret = MPI_Allreduce(rank_newClusterSize, newClusterSize, numClusters, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
        if (ret != MPI_SUCCESS)
        {
            printf("Error: Cluster size allreduce failed!\n");
            MPI_Finalize();
            exit(1);
        }
         ...
         */
        ret = MPI_Allreduce(&rank_delta, &delta, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        if (ret != MPI_SUCCESS)
        {
            printf("Error: Delta allreduce failed!\n");
            MPI_Finalize();
            exit(1);
        }
    ...
    timing = wtime() - timing;
    if (rank == 0)
        fprintf(stdout, "        nloops = %3d   (total = %7.4fs)  (per loop = %7.4fs)\n", loop, timing, timing / loop);
